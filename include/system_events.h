...
namespace system_input
{
    ...
    struct Keyboard
    {
        ...
        struct KeyState
        {
            void reset() { key = None; state.reset(); }

            bool clicked(Key what, uint8_t times = 1) const { return key == what && !state.pressed && state.clicks == times; }
            bool operator()(Key what, uint8_t times = 1) const { return key == what && !state.pressed && state.clicks == times; }
    ...
    public:
        ...
        void key(uint8_t u, bool down)
        {
            Time time = Clock::now();
            auto const dt = time - keys_time;
            Keyboard::Key const k = Keyboard::Key(u);
            if(summary.keys.pressed[k] == down)
                return;
            //keys_prev = summary.keys.pressed;
            summary.keys.pressed.flip(k);
            if(k != summary.keys.clicked.key || dt > click_threshold)
            {
                summary.mouse.clicked.state.reset(down);
            }
            else if(!down)
            ...
            for(uint8_t x = 0; x != 255 ; ++x)
                if(summary.keys.pressed[x])
                    std::cout << std::format("{}({}) ", x, int(x));
            std::cout << std::endl;
        }
